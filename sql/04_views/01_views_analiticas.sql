...
SELECT ano, COUNT(*) AS total_acidentes, SUM(mortos) AS mortos, SUM(feridos_graves) AS feridos_graves
FROM tratado.ocorrencias
GROUP BY ano;

CREATE OR REPLACE VIEW modelado.vw_acidentes_por_uf AS
SELECT uf, COUNT(*) AS total_acidentes, SUM(mortos) AS mortos, SUM(feridos_graves) AS feridos_graves
FROM tratado.ocorrencias
GROUP BY uf;

CREATE OR REPLACE VIEW modelado.vw_acidentes_por_br AS
SELECT br, COUNT(*) AS total_acidentes, SUM(mortos) AS mortos, SUM(feridos_graves) AS feridos_graves
FROM tratado.ocorrencias
GROUP BY br;
...
SELECT ano, COUNT(*) AS total_acidentes, SUM(mortos) AS mortos, SUM(total_vitimas) AS total_vitimas
FROM tratado.ocorrencias
GROUP BY ano
ORDER BY ano;
